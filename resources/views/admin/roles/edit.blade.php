...
<div class="row">
    <div class="col-lg-12 col-xs-12 col-sm-12 col-md-12">
        <form class="card" method="POST" action="{{route('admin'.$ruta.'update' , $row->id)}}" enctype="multipart/form-data" class="submit-prevent-form">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
                ...
                <div class="row row-cards">
                    ...
                    <div class="col-sm-6 col-md-6">
                      <div class="mb-3">
                        ...
                        <input type="text" name="description" class="form-control" id="name" placeholder="Descripción del rol" value=" {{ $row->description }}" >
                      </div>
                    </div>
                        ...
                        <div class="card w-100">
                            ...
                            <div class="card-actions">
                                ...
                                <div class="float-end">
                                    ...
                                    <input type="checkbox" class="form-check-input switch-permission" id="switch-{!! str_replace(' ', '', $key) !!}"
...
<script>
    ...
    function verifyCheckbox(){
        let prueba = document.getElementsByClassName('all-permiso');
        var permisos = @json($group_names_roles);
        //console.log("prueba",prueba);

        permisos.forEach(name => {
            for (let i = 0; i < prueba.length; i++) {
                const element = prueba[i];
                if ( element.classList[2] == `permiso-${name}` && element.checked == true ) {
                    $(`#switch-${name}`).prop('checked', true);
                }
            }
